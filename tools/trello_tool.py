...
class TrelloTools(Toolkit):
    ...
    def __init__(
        ...
        if info_for_all_boards:
            self.register(self.info_for_all_boards)
        if list_boards:
    ...
    def info_for_all_boards(self, actions: Optional[str] = "all") -> str:
        """
        Get information for all boards in one request.

        Args:
            actions (str, optional): Type of actions to retrieve. Defaults to "all".

        Returns:
            str: JSON string containing detailed information about all boards
        """
        try:
            if not self.client:
                return "Trello client not initialized"
            ...
            # Process and structure the board information
            boards_info = []
            for board in self.client.all_info:
                board_data = {
                    "id": board.get("id"),
                    "name": board.get("name"),
                    "desc": board.get("desc"),
                    ...
                    "members_count": len(board.get("memberships", [])),
                    "lists_count": len(board.get("lists", [])),
                }
                boards_info.append(board_data)

            return json.dumps({
                "total_boards": len(boards_info),
                "boards": boards_info
            })

        except Exception as e:
            return f"Error retrieving board information: {e}"
